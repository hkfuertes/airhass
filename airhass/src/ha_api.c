...
bool ha_build_now_playing_payload(const char *state, const char *artist, const char *album,
	...
	if (artist && *artist) json_object_set_new_nocheck(attrs, "media_artist", json_string(artist));
	if (album && *album) json_object_set_new_nocheck(attrs, "media_album_name", json_string(album));
	if (title && *title) json_object_set_new_nocheck(attrs, "media_title", json_string(title));
...
bool ha_set_now_playing(const char *url, const char *token, const char *entity_id, const char *state,
	...
	char path[320], payload[2048];

	if (!entity_id || !*entity_id || !state || !*state) return false;
	if (snprintf(path, sizeof(path), "/api/states/%s", entity_id) >= (int) sizeof(path)) return false;
	if (!ha_build_now_playing_payload(state, artist, album, title, entity_picture, payload, sizeof(payload))) {
